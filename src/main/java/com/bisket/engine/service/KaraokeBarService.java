...
public class KaraokeBarService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<KaraokeBar> parsedList = KaraokeBarParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, KaraokeBar> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<KaraokeBar> foundList = karaokeBarRepository.findAll();
            if (!foundList.isEmpty()) {
                for (KaraokeBar found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                KaraokeBar parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    KaraokeBar found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    karaokeBarRepository.save(parsed);
                }
            }
        }
