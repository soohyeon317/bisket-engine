...
public class PhysicalEducationGymService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<PhysicalEducationGym> parsedList = PhysicalEducationGymParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, PhysicalEducationGym> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<PhysicalEducationGym> foundList = physicalEducationGymRepository.findAll();
            if (!foundList.isEmpty()) {
                for (PhysicalEducationGym found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                PhysicalEducationGym parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    PhysicalEducationGym found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    physicalEducationGymRepository.save(parsed);
                }
            }
        }
