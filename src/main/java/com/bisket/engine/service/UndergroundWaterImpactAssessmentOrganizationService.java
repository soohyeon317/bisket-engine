...
public class UndergroundWaterImpactAssessmentOrganizationService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<UndergroundWaterImpactAssessmentOrganization> parsedList = UndergroundWaterImpactAssessmentOrganizationParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, UndergroundWaterImpactAssessmentOrganization> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<UndergroundWaterImpactAssessmentOrganization> foundList = undergroundWaterImpactAssessmentOrganizationRepository.findAll();
            if (!foundList.isEmpty()) {
                for (UndergroundWaterImpactAssessmentOrganization found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                UndergroundWaterImpactAssessmentOrganization parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    UndergroundWaterImpactAssessmentOrganization found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    undergroundWaterImpactAssessmentOrganizationRepository.save(parsed);
                }
            }
        }
