...
public class GameProductionService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<GameProduction> parsedList = GameProductionParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, GameProduction> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<GameProduction> foundList = gameProductionRepository.findAll();
            if (!foundList.isEmpty()) {
                for (GameProduction found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                GameProduction parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    GameProduction found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    gameProductionRepository.save(parsed);
                }
            }
        }
