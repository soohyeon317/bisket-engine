...
public class TourTrackService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<TourTrack> parsedList = TourTrackParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, TourTrack> compositeKeyToFoundObjectMap = new HashMap<>();
            List<TourTrack> foundList = tourTrackRepository.findAll();
            if (!foundList.isEmpty()) {
                for (TourTrack found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                TourTrack parsed = parsedList.get(i);
                ...
                if (compositeKeyToFoundObjectMap.containsKey(compositeKey)) {
                    /* 업데이트 진행 */
                    TourTrack found = compositeKeyToFoundObjectMap.get(compositeKey);
                    ...
                    /* 인서트 진행 */
                    tourTrackRepository.save(parsed);
                }
            }
        }
