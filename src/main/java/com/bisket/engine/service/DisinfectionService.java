...
public class DisinfectionService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<Disinfection> parsedList = DisinfectionParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, Disinfection> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<Disinfection> foundList = disinfectionRepository.findAll();
            if (!foundList.isEmpty()) {
                for (Disinfection found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                Disinfection parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    Disinfection found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    disinfectionRepository.save(parsed);
                }
            }
        }
