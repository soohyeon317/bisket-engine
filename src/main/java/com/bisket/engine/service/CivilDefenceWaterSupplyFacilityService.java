...
public class CivilDefenceWaterSupplyFacilityService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        if (!parsedList.isEmpty()) {
            Map<String, CivilDefenceWaterSupplyFacility> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    CivilDefenceWaterSupplyFacility found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
