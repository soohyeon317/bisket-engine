...
public class PetroleumSaleService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<PetroleumSale> parsedList = PetroleumSaleParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, PetroleumSale> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<PetroleumSale> foundList = petroleumSaleRepository.findAll();
            if (!foundList.isEmpty()) {
                for (PetroleumSale found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                PetroleumSale parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    PetroleumSale found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    petroleumSaleRepository.save(parsed);
                }
            }
        }
