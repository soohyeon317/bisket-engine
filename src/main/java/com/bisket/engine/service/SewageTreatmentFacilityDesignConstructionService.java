...
public class SewageTreatmentFacilityDesignConstructionService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        if (!parsedList.isEmpty()) {
            Map<String, SewageTreatmentFacilityDesignConstruction> compositeKeyToFoundObjectMap = new HashMap<>();
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                ...
                if (compositeKeyToFoundObjectMap.containsKey(compositeKey)) {
                    /* 업데이트 진행 */
                    SewageTreatmentFacilityDesignConstruction found = compositeKeyToFoundObjectMap.get(compositeKey);
