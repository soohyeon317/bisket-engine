...
public class MotorCampService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<MotorCamp> parsedList = MotorCampParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, MotorCamp> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<MotorCamp> foundList = motorCampRepository.findAll();
            if (!foundList.isEmpty()) {
                for (MotorCamp found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                MotorCamp parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    MotorCamp found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    motorCampRepository.save(parsed);
                }
            }
        }
