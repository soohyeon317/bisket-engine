...
public class LivestockExcretaCollectionTransportService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<LivestockExcretaCollectionTransport> parsedList = LivestockExcretaCollectionTransportParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, LivestockExcretaCollectionTransport> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<LivestockExcretaCollectionTransport> foundList = livestockExcretaCollectionTransportRepository.findAll();
            if (!foundList.isEmpty()) {
                for (LivestockExcretaCollectionTransport found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                LivestockExcretaCollectionTransport parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    LivestockExcretaCollectionTransport found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    livestockExcretaCollectionTransportRepository.save(parsed);
                }
            }
        }
