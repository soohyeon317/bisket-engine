...
public class ColdCallingSaleService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<ColdCallingSale> parsedList = ColdCallingSaleParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, ColdCallingSale> compositeKeyToFoundObjectMap = new HashMap<>();
            List<ColdCallingSale> foundList = coldCallingSaleRepository.findAll();
            if (!foundList.isEmpty()) {
                for (ColdCallingSale found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                ColdCallingSale parsed = parsedList.get(i);
                ...
                if (compositeKeyToFoundObjectMap.containsKey(compositeKey)) {
                    /* 업데이트 진행 */
                    ColdCallingSale found = compositeKeyToFoundObjectMap.get(compositeKey);
                    ...
                    /* 인서트 진행 */
                    coldCallingSaleRepository.save(parsed);
                }
            }
        }
