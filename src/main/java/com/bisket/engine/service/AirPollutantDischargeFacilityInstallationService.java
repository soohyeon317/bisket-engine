...
public class AirPollutantDischargeFacilityInstallationService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        if (!parsedList.isEmpty()) {
            Map<String, AirPollutantDischargeFacilityInstallation> compositeKeyToFoundObjectMap = new HashMap<>();
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                ...
                if (compositeKeyToFoundObjectMap.containsKey(compositeKey)) {
                    /* 업데이트 진행 */
                    AirPollutantDischargeFacilityInstallation found = compositeKeyToFoundObjectMap.get(compositeKey);
