...
public class MusicRecordProductionService implements BusinessBaseService {
    ...
    public Integer updateListFromXmlFile(String filePath) throws Exception {
        ...
        List<MusicRecordProduction> parsedList = MusicRecordProductionParser.getListFromXml(xml);

        if (!parsedList.isEmpty()) {
            Map<String, MusicRecordProduction> compositeUniqueKeyToFoundObjectMap = new HashMap<>();
            List<MusicRecordProduction> foundList = musicRecordProductionRepository.findAll();
            if (!foundList.isEmpty()) {
                for (MusicRecordProduction found : foundList) {
            ...
            for (int i = 0; i < parsedList.size(); i++) {
                MusicRecordProduction parsed = parsedList.get(i);
                ...
                if (compositeUniqueKeyToFoundObjectMap.containsKey(compositeUniqueKey)) {
                    /* 업데이트 진행 */
                    MusicRecordProduction found = compositeUniqueKeyToFoundObjectMap.get(compositeUniqueKey);
                    ...
                    /* 인서트 진행 */
                    musicRecordProductionRepository.save(parsed);
                }
            }
        }
